...
function showExportDialog() {
  ...
  addRadioButton = (name, size, ...descriptions) => {
    const container = createAndAppendDivWithClass(dialog, 'menu-radio-group');
...
function showImportDialog() {
  ...
  addRadioButton = (name, ...descriptions) => {
    ...
    button.name = 'import';
    button.value = name;
    const id = 'exportButton' + importButtons.length;
    button.id = id;
    container.appendChild(button);
    importButtons.push(button);
    if (importButtons.length == 1) button.checked = true;
    const label = document.createElement('label');
    label.innerHTML = `<b>${name}</b><br />` +
        `<div class="menu-radio-details">${descriptions.join('<br />')}</div>`;
    label.setAttribute('for', id);
    container.appendChild(label);
  };
