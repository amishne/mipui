class State {
  ...
  cellKeyIsInTheMap_(cellKey) {
    const cell = this.theMap.cells.get(cellKey);
    if (!cell) return true;
    return cell.column >= this.getProperty(pk.firstColumn) - 0.5 &&
        cell.column < this.getProperty(pk.lastColumn) &&
        cell.row >= this.getProperty(pk.firstRow) - 0.5 &&
        cell.row < this.getProperty(pk.lastRow);
  }

  deleteContentNotInTheMap() {
    const keysToDelete = new Set();
    const startCellKeysRemoved = new Map();
    ct.children.forEach(layer => {
      startCellKeysRemoved.set(layer.id, new Set());
    });
    Object.keys(this.pstate_.content).forEach(key => {
      // If this key is no longer in the map, mark it for deletion.
      if (!this.cellKeyIsInTheMap_(key)) keysToDelete.add(key);
      // If any endCell of this key is no longer in the map, mark that layer on
      // this cell (and its non-start cells) for deletion.
      Object.keys(this.pstate_.content[key]).forEach(layerId => {
        const endCellKey = this.pstate_.content[key][layerId][ck.endCell];
        if (!endCellKey) return;
        if (!this.cellKeyIsInTheMap_(endCellKey)) {
          startCellKeysRemoved.get(Number.parseInt(layerId)).add(key);
        }
      });
    });
    keysToDelete.forEach(key => {
      Object.keys(this.pstate_.content[key]).forEach(layerId => {
        const content = this.pstate_.content[key][layerId];
        delete this.pstate_.content[key][layerId];
        state.opCenter.recordCellChange(
            key, layerId, content, null);
      });
      delete this.pstate_.content[key];
    });
    Object.keys(this.pstate_.content).forEach(key => {
      Object.keys(this.pstate_.content[key]).forEach(layerId => {
        const content = this.pstate_.content[key][layerId];
        const removedSet = startCellKeysRemoved.get(Number.parseInt(layerId));
        let deleteLayer = false;
        if (removedSet.has(key)) {
          deleteLayer = true;
        } else {
          const startCellKey = content[ck.startCell];
          deleteLayer = startCellKey &&
              (keysToDelete.has(startCellKey) || removedSet.has(startCellKey));
        }
        if (deleteLayer) {
          delete this.pstate_.content[key][layerId];
          state.opCenter.recordCellChange(
